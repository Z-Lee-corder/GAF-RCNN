...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        input1=pooled_features_list[0].view(batch_size,-1, pooled_features_list[0].shape[-1])
        input2=pooled_features_list[1].view(batch_size,-1, pooled_features_list[1].shape[-1])
        input1=input1.permute(0,2,1)
        input2=input2.permute(0,2,1)
        xa=input1+input2
        x1=self.local_att(xa)
        xg=self.global_att(xa)
        x1g=x1+xg
        wei=self.sigmoid(x1g)
        x0=2*input1*wei+2*input2*(1-wei)
        ms_pooled_features = x0.permute(0,2,1).view(batch_size*128,216,128)
