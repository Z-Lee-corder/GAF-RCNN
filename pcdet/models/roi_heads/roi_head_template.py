...
class RoIHeadTemplate(nn.Module):
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == 'smooth-l1':
        ...
        elif self.model_cfg.LOSS_CONFIG.REG_LOSS == 'CIOU_loss':
            pos_mask = self.forward_ret_dict['rcnn_cls_labels'] > 0
            reg_weights = (pos_mask.float()).view(-1)
            pos_normalizer = pos_mask.sum().float()
            reg_weights /= torch.clamp(pos_normalizer, min=1.0)
            rois = self.forward_ret_dict['rois']
            bboxes1 = rois.view(-1, 7)
            gt_of_rois = self.forward_ret_dict['gt_of_rois_src']
            bboxes2 = gt_of_rois[:, :, 0:7].view(-1, 7)
            bboxes1 = torch.where(torch.isnan(bboxes1), bboxes2, bboxes1)
            ...
            u = (inter_diag)/(c_diag)
            iou = (self.forward_ret_dict['gt_iou_of_rois']).view(-1)
            dious = iou - u - 1.25 * (1 - torch.abs(torch.cos(angle1 - angle2)))
            dious = torch.clamp(dious, min=-1.0, max=1.0)
            rcnn_loss_reg = 1 - dious
            rcnn_loss_reg = (rcnn_loss_reg * reg_weights).sum()

            loss_weights_dict = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS
            rcnn_loss_reg = rcnn_loss_reg * loss_weights_dict['rcnn_reg_weight']
            if tb_dict is None:
                tb_dict = {}
            tb_dict.update({'rcnn_loss_reg': rcnn_loss_reg.item()})
        else:
            raise NotImplementedError
